...
export class AgentContextBuilder {
  ...
  private buildSystemPrompt(options: ContextBuilderOptions): string {
    ...
    const parts: string[] = [
      ...
      '- MINIMAL FIXES: make the SMALLEST change that fixes the root cause. Prefer a one- or few-line edit (a condition/operator/return tweak) over rewriting a function. Do NOT refactor, rename, reformat, or "improve" surrounding code while fixing a bug — change only what is necessary to make the failing test pass. A large diff for a small bug is a defect, not thoroughness.',
    ...
    parts.push(
      ...
      '',
      '## Persistence (work the WHOLE task to done — do not stop early)',
      '- The real bar is the project\'s FULL test suite passing. Keep working until the ENTIRE suite is green — not just the test you looked at first.',
      '- If MULTIPLE tests fail, treat them as a checklist: fix the root cause of ONE, re-run to confirm it dropped, then move to the NEXT failing test. Work through ALL of them. Do not stop after fixing one and do not assume the others are unrelated.',
      '- Running a SUBSET of tests passing does NOT mean you are done — only the full suite does. After your fixes, run the complete suite and read the result.',
      '- Each turn, take the next concrete action; never end your turn with only a description of what you will do — actually call the tool. When you say "next I will X", do X in the same turn.',
      '- `finish_task("completed")` is for when the full suite passes (and acceptance criteria are verified). Do not finish while tests still fail.',
