...
// opencode --format json emits a STREAM of step_finish events, each with a per-step
// tokens object {input, output, reasoning, cache:{read}}. Sum input+output across all
// steps for real metered tokens; cache.read summed separately (comparable to Forge's
// Main-model tokens / Cache-read split). One JSON.parse of the whole stream cannot work.
function parseOpencodeTokens(out) {
  const m = [...out.matchAll(/"tokens":\{"total":\d+,"input":(\d+),"output":(\d+),"reasoning":\d+,"cache":\{"write":\d+,"read":(\d+)\}/g)]
  if (!m.length) return null
  const sum = (i) => m.reduce((a, x) => a + Number(x[i]), 0)
  return { inputTokens: sum(1), outputTokens: sum(2), calls: m.length, cacheReadTokens: sum(3) }
}
function runOpencode() {
  ...
  return new Promise((resolve) => {
    ...
    const done = (code) => {
      ...
      resolve({ exitCode: code ?? 1, runtimeMs: Date.now() - start, tokens: parseOpencodeTokens(out), iterations: 0 })
    }
    child.on('close', done)
